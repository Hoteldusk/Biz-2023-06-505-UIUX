...
// 자바스크립티에서 처음으로 사용하기 시작한 독특한 data 객체
...
document.addEventListener("DOMContentLoaded", () => {
    const join = () => {
        ...
        if(!join_input[INPUT_INDEX.USERNAME].value) {
            ...
            join_input[INPUT_INDEX.USERNAME].focus();   
            return false;
        }

        const input_password = join_input[INPUT_INDEX.PASSWORD].value
        
        if(!join_input[INPUT_INDEX.PASSWORD].value) {
            alert("PASSWORD 은 필수 항목 입니다")
            join_input[INPUT_INDEX.PASSWORD].focus();   
            return false;
        }

        // 정규화 문자열 : 일반 문자열과 달리 / 로 감싸진 문자열이다
        // [] : OR, [A-Z] : 입력된 문자열이 대문자 A ~ Z 중에 있는가?
        // [A-Z]{3} : 대문자 3자리 문자열인가 검사
        // 특수문자, 영문대소문자, 숫자 포함하여 8 ~ 15 자리 이내
        const password_check = /^.*(?=^.{8,15}$)(?=.*\d)(?=.*[a-zA-Z])(?=.*[!@#$%^&+=]).*$/
        if(!password_check.test(input_password)) {
            alert("비밀번호는 특수문자, 영문 대소문자, 숫자 포함하여 8 ~ 15자리 형식으로 입력하세요");
            join_input[INPUT_INDEX.PASSWORD].focus();
            return false;   
        }
    ...
    document.querySelector("#join_button")
